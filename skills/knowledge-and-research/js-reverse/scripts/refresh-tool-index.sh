...

{
  ...
  echo "- Script: \`scripts/refresh-tool-index.sh\`"
...
# Parity with scripts/refresh-tool-index.ps1 (the "能力状态视图" block).
# Computed by parsing scripts/bootstrap-manifest.json plus probing the
...
with open(output_json, 'w', encoding='utf-8') as f:
    json.dump({
        ...
        'platform': platform,
        'uname': uname,
        'script': 'scripts/refresh-tool-index.sh',
        'tools': tools,
