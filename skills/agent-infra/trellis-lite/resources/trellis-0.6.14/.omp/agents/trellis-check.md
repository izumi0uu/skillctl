...
Resolve the active task from the injected `Task Identity` or the dispatch
prompt's required `Active task: <path>` header. If neither is present, ask the
main session; do not guess or use `task.py current` from the child session.

Read the active task's `prd.md`, optional `design.md` and `implement.md`, and
valid spec/research entries from `check.jsonl`. Any omitted or truncated entry
in the injected file manifest must be read from disk before relying on it.
Product code and tests are read on demand; JSONL is not a source manifest.
